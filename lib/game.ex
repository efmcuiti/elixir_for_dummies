defmodule IslandsEngine.Game do
  use GenServer
  alias IslandsEngine.{Coordinate, Game, Player, Rules}
  ...
  defp guess_reply(:ok, oponent_board, coordinate) do
  ...
  defp forest_check(:miss, _oponent, _coordinate) do
    {:miss, :none}
  end
  
  defp forest_check(:hit, oponent, coordinate) do
    island_key = Player.forested_island(oponent, coordinate)
    {:hit, island_key}
  end

  defp forest_check({:error, :action_out_of_sequence}, _oponent_board, _coordinate) do
    {:error, :action_out_of_sequence}
  end

  defp win_check({hit_or_miss, :none}, _oponent, state) do
    {:reply, {hit_or_miss, :none, :no_win}, state}
  end

  defp win_check({:hit, island_key}, oponent, state) do
    win_status =
      case Player.win?(oponent) do
        true -> 
          Rules.win(state.fsm)
          :win
        false -> :no_win
  ...
  defp win_check({:error, :action_out_of_sequence}, _oponent, state) do
    {:reply, {:error, :action_out_of_sequence}, state}
  end

  defp oponent(state, :player1) do
    state.player2
  end

  defp oponent(state, _player2) do
    state.player1
  end
end
